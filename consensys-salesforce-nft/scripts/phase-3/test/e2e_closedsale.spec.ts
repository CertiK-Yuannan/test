...
import { ERC721_CS_V2, PAYMENT_SPLITTER, ClosedSaleParams, nonceGen } from "../common";
...
describe(`${ERC721_CS_V2} - Happy Path E2E`, () => {
  let nft: any;
  ...
  it("should mint 3 tokens with tokenId=0,1,2", async () => {
  ...
  it("should generate correct tokenURIs for the minted tokens", async () => {
    expect(await nft.tokenURI(0)).toEqual(`${saleParams.baseURI}`);
    expect(await nft.tokenURI(1)).toEqual(`${saleParams.baseURI}`);
    expect(await nft.tokenURI(2)).toEqual(`${saleParams.baseURI}`);
  });
  ...
  it("should reveal the sale", async () => {
    const baseURI = "some-base-uri";

    expect(new BN(await nft.offsetIndex()).eq(new BN(0))).toEqual(true);
    expect(await nft.tokenURI(0)).not.toContain(baseURI);

    nonce = multisigNonceGen();
    const payload = createRevealSalePayload(erc721Address, chainId, nonce, baseURI);
    const multiSigParam = await createMultiSigParam(payload, nonce, contractAdminSigners);
    const receipt = await nft.revealSale(baseURI, multiSigParam, { from: defaultSender });
    expectEvent(receipt, "SaleRevealed");
    expect(new BN(await nft.offsetIndex()).gte(new BN(0))).toEqual(true);
    expect(await nft.tokenURI(0)).toContain(baseURI);
  });

  it("should generate correct tokenURIs with offsetIndexIds for the minted tokens, after a reveal", async () => {
    const offsetIndexId = (await nft.offsetIndex()).toNumber();
    const newBaseURI = await nft.baseURI();
    expect(await nft.tokenURI(0)).toEqual(`${newBaseURI}${0 + offsetIndexId}`);
    expect(await nft.tokenURI(1)).toEqual(`${newBaseURI}${1 + offsetIndexId}`);
    expect(await nft.tokenURI(2)).toEqual(`${newBaseURI}${2 + offsetIndexId}`);
  });
