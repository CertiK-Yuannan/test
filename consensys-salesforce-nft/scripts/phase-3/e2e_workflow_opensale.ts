...
async function deployNft() {
  ...
  params = createOpenSaleParams(accounts, multiSigRoleThreshold());
...
async function mintTokens() {
  ...
  for (let tokenId = 0; tokenId < 3; tokenId++) {
    ...
    const txn = await buyerContract.mintToken(tokenId, voucherParam, { value });
    await txn.wait();
  }
